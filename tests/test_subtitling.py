...
from but_with_subs.constants import MIN_CHUNK_DISPLAY_LENGTH_SECONDS, OVERLAPPING_SPEAKER_COLORS
...
class TestEscapeVttText:
    ...
    def test_escaped_ampersand_first(self):
...
class TestDetectOverlappingSpeakers:
    ...
    def test_single_chunk(self):
        ...
        chunks = [
            Chunk(start_time=0.0, end_time=2.0, audio=np.array([1]), text="test", speaker="Alice"),
        ]
    ...
    def test_no_overlap(self):
        ...
        chunks = [
            Chunk(start_time=0.0, end_time=2.0, audio=np.array([1]), text="test", speaker="Alice"),
            Chunk(start_time=3.0, end_time=5.0, audio=np.array([1]), text="test", speaker="Bob"),
        ]
        result = _detect_overlapping_speakers(chunks)
        assert result == {}
    ...
    def test_simple_overlap(self):
        ...
        chunks = [
            Chunk(start_time=0.0, end_time=3.0, audio=np.array([1]), text="test", speaker="Alice"),
            Chunk(start_time=2.0, end_time=5.0, audio=np.array([1]), text="test", speaker="Bob"),
        ]
        result = _detect_overlapping_speakers(chunks)
    ...
    def test_three_way_overlap(self):
        ...
        chunks = [
            Chunk(start_time=0.0, end_time=4.0, audio=np.array([1]), text="test", speaker="Alice"),
            Chunk(start_time=2.0, end_time=5.0, audio=np.array([1]), text="test", speaker="Bob"),
            Chunk(start_time=3.0, end_time=6.0, audio=np.array([1]), text="test", speaker="Charlie"),
        ]
        result = _detect_overlapping_speakers(chunks)
        assert "Alice" in result
        assert "Bob" in result
    ...
    def test_chunks_without_speaker(self):
        ...
        chunks = [
            Chunk(start_time=0.0, end_time=2.0, audio=np.array([1]), text="test", speaker=None),
            Chunk(start_time=1.5, end_time=3.0, audio=np.array([1]), text="test", speaker="Alice"),
        ]
        result = _detect_overlapping_speakers(chunks)
        assert result == {}
    ...
    def test_touching_not_overlapping(self):
        ...
        chunks = [
            Chunk(start_time=0.0, end_time=2.0, audio=np.array([1]), text="test", speaker="Alice"),
            Chunk(start_time=2.0, end_time=4.0, audio=np.array([1]), text="test", speaker="Bob"),
        ]
        result = _detect_overlapping_speakers(chunks)
        assert result == {}
...
class TestAssignSpeakerColors:
    ...
    def test_sorted_by_first_overlap(self):
...
class TestApplySpeakerColor:
    ...
    def test_basic_color_application(self):
...
class TestGenerateSubtitles:
    ...
    def test_generate_subtitles_creates_file(self, sample_chunks, temp_audio_file):
    ...
    def test_generate_subtitles_output_location(self, sample_chunks, temp_audio_file):
        """Test that output file is in the same directory as audio."""
        result_path = generate_subtitles(sample_chunks, temp_audio_file)
        expected_path = temp_audio_file.with_suffix(".vtt")
        assert result_path == expected_path

    def test_generate_subtitles_content(self, sample_chunks, temp_audio_file):
    ...
    def test_generate_subtitles_uses_min_duration(self, temp_audio_file):
        ...
        content = result_path.read_text(encoding="utf-8")
        assert "00:00:00.000 --> 00:00:00.500" in content or "00:00:00.000 --> 00:00:00.600" in content

    def test_generate_subtitles_sorts_by_time(self, temp_audio_file):
        """Test that chunks are sorted by start time in output."""
        chunks = [
            Chunk(start_time=5.0, end_time=7.0, audio=np.array([1]), text="Second", speaker="Bob"),
            Chunk(start_time=1.0, end_time=3.0, audio=np.array([1]), text="First", speaker="Alice"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        content = result_path.read_text(encoding="utf-8")
    ...
    def test_generate_subtitles_escapes_special_chars(self, temp_audio_file):
    ...
    def test_generate_subtitles_handles_none_speaker(self, temp_audio_file):
        """Test handling of chunks without speaker information."""
        chunks = [
            Chunk(start_time=0.0, end_time=2.0, audio=np.array([1]), text="No speaker", speaker=None),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        content = result_path.read_text(encoding="utf-8")
    ...
    def test_generate_subtitles_applies_colors_for_overlapping(self, overlapping_chunks, temp_audio_file):
    ...
    def test_generate_subtitles_uses_correct_indexing(self, sample_chunks, temp_audio_file):
...
class TestEdgeCases:
    ...
    def test_empty_text_chunk(self, temp_audio_file):
        """Test handling of chunks with empty text."""
        chunks = [
            Chunk(start_time=0.0, end_time=2.0, audio=np.array([1]), text="", speaker="Alice"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        assert result_path.exists()

    def test_special_unicode_characters(self, temp_audio_file):
    ...
    def test_very_long_text(self, temp_audio_file):
        """Test handling of very long text."""
        long_text = " ".join(["word"] * 100)
        chunks = [
            Chunk(start_time=0.0, end_time=10.0, audio=np.array([1]), text=long_text, speaker="Alice"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        assert result_path.exists()

    def test_very_long_duration(self, temp_audio_file):
        """Test handling of very long duration timestamps."""
        chunks = [
            Chunk(start_time=0.0, end_time=7200.0, audio=np.array([1]), text="Long", speaker="Alice"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        content = result_path.read_text(encoding="utf-8")
        assert "02:" in content

    def test_negative_timestamp_handling(self, temp_audio_file):
        """Test handling of negative start times."""
        chunks = [
            Chunk(start_time=-1.0, end_time=1.0, audio=np.array([1]), text="Starts before", speaker="Alice"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        assert result_path.exists()

    def test_identical_start_end_times(self, temp_audio_file):
        """Test handling of chunks where start equals end."""
        chunks = [
            Chunk(start_time=5.0, end_time=5.0, audio=np.array([1]), text="Instant", speaker="Alice"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        content = result_path.read_text(encoding="utf-8")
        assert "00:00:05.000 --> 00:00:05.500" in content

    def test_overlapping_same_speaker(self, temp_audio_file):
        """Test handling when same speaker overlaps with themselves."""
        chunks = [
            Chunk(start_time=0.0, end_time=3.0, audio=np.array([1]), text="Part 1", speaker="Alice"),
            Chunk(start_time=2.0, end_time=5.0, audio=np.array([1]), text="Part 2", speaker="Alice"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        assert result_path.exists()

    def test_many_chunks(self, temp_audio_file):
        """Test handling of many chunks."""
        chunks = [
            Chunk(start_time=i * 0.5, end_time=(i + 1) * 0.5, audio=np.array([1]), text=f"Chunk {i}", speaker=f"Speaker {i % 3}")
            for i in range(100)
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        content = result_path.read_text(encoding="utf-8")
    ...
    def test_file_overwritten_if_exists(self, sample_chunks, temp_audio_file):
        """Test that existing subtitle file is overwritten."""
        generate_subtitles(sample_chunks, temp_audio_file)
        
        modified_chunks = [
            Chunk(start_time=0.0, end_time=1.0, audio=np.array([1]), text="Modified", speaker="Charlie"),
        ]
...
class TestErrorHandling:
    ...
    def test_empty_chunks_raises_error(self, temp_audio_file):
    ...
    def test_nonexistent_audio_file_raises_error(self, sample_chunks):
    ...
    def test_directory_as_audio_path_creates_in_dir(self, sample_chunks, tmp_path):
...
class TestStressScenarios:
    ...
    def test_rapid_fire_chunks(self, temp_audio_file):
        """Test with many very short chunks in rapid succession."""
        chunks = [
            Chunk(start_time=i * 0.1, end_time=(i + 1) * 0.1, audio=np.array([1]), text=f"W{i}", speaker=f"S{i % 5}")
            for i in range(50)
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        assert result_path.exists()

    def test_wide_time_range(self, temp_audio_file):
        """Test with chunks spanning a wide time range."""
        chunks = [
            Chunk(start_time=0.0, end_time=1.0, audio=np.array([1]), text="Start", speaker="Alice"),
            Chunk(start_time=1000.0, end_time=1001.0, audio=np.array([1]), text="Middle", speaker="Bob"),
            Chunk(start_time=10000.0, end_time=10001.0, audio=np.array([1]), text="End", speaker="Charlie"),
        ]
        result_path = generate_subtitles(chunks, temp_audio_file)
        content = result_path.read_text(encoding="utf-8")
