...
from but_with_subs.translation import Translator, translate_subtitles
...
class TestCompletePipeline:
    ...
    def test_full_pipeline_audio_to_translated_subtitles(
        ...
        with patch("but_with_subs.transcribing._transcribe_chunks_batch") as mock_transcribe:
            mock_transcribe.return_value = [
                [
                    Chunk(start_time=0.0, end_time=0.5, audio=np.zeros(8000), text="Hej", speaker="Alice"),
                    Chunk(start_time=0.5, end_time=1.0, audio=np.zeros(8000), text="verden", speaker="Alice"),
                    Chunk(start_time=1.0, end_time=1.5, audio=np.zeros(8000), text="hvad", speaker="Alice"),
                    Chunk(start_time=1.5, end_time=2.0, audio=np.zeros(8000), text="så", speaker="Alice"),
                ],
                [
                    Chunk(start_time=2.5, end_time=3.5, audio=np.zeros(16000), text="Jeg", speaker="Bob"),
                    Chunk(start_time=3.5, end_time=4.5, audio=np.zeros(16000), text="hedder", speaker="Bob"),
                    Chunk(start_time=4.5, end_time=5.0, audio=np.zeros(8000), text="Bob", speaker="Bob"),
                ],
            ]
            transcribed = transcribe_chunks_dynamic(chunks, mock_asr_model, show_progress=False)
        ...
        with patch("but_with_subs.text_chunking.PunctFixer") as mock_punctfixer_class:
            ...
            grouped_chunks = group_word_chunks(all_word_chunks, mock_punctfixer, max_words=5)
        ...
        with patch("but_with_subs.translation.pipeline", return_value=mock_translate_pipeline):
...
class TestPipelineDataIntegrity:
    ...
    def test_timing_info_preserved_through_pipeline(
        ...
        original_times = [(c.start_time, c.end_time) for c in mock_word_chunks]
...
class TestModuleInteractions:
    ...
    def test_audio_loading_to_chunking_integration(self, mock_audio_chunks: list[Chunk]) -> None:
...
class TestRealWorldScenarios:
    ...
    def test_multilingual_content_scenario(self, tmp_path: Path) -> None:
        ...
        with patch("but_with_subs.translation.pipeline", return_value=mock_translate_en):
        ...
        with patch("but_with_subs.translation.pipeline", return_value=mock_translate_de):
...
class TestPipelineErrorHandling:
    ...
    def test_transcription_failure_handling(self, mock_audio_chunks: list[Chunk]) -> None:
        ...
        with pytest.raises(RuntimeError, match="ASR model failed"):
            transcribe_chunks_dynamic(mock_audio_chunks, mock_asr_model, show_progress=False)
    ...
    def test_mocked_batch_transcription(self, mock_audio_chunks: list[Chunk]) -> None:
        ...
        def mock_batch_call(audio_list, return_timestamps=None):
            ...
            for i, audio in enumerate(audio_list):
                results.append({
                    "chunks": [
                        {"text": f"Transcript {i}", "timestamp": (0.0, len(audio) / 16000)},
                    ]
                })
            return results
...
class TestDataFlowVerification:
    ...
    def test_chunk_data_flow(self) -> None:
        ...
        assert original[0].start_time == transcribed[0].start_time == translated[0].start_time
    ...
    def test_timestamp_flow_accuracy(self, mock_word_chunks: list[Chunk]) -> None:
        ...
        for g_chunk in grouped:
            ...
            overlapping = [
                o for o in mock_word_chunks
                if not (g_chunk.end_time < o.start_time or g_chunk.start_time > o.end_time)
            ]
            assert len(overlapping) > 0, f"No overlapping original chunk found for {g_chunk.start_time}-{g_chunk.end_time}"
