...
class TestCUDADeviceDetection:
    ...
    def test_cuda_device_string_is_correct(
        self,
    ) -> None:
...
class TestMPSDeviceDetection:
    ...
    def test_mps_available_when_cuda_not_available(
        self,
    ) -> None:
        """Test that MPS is used when CUDA is not available but MPS is."""
        with patch("but_with_subs.device.torch.cuda") as mock_cuda, patch(
            "but_with_subs.device.torch.backends"
        ) as mock_backends:
    ...
    def test_mps_device_string_is_correct(
        self,
    ) -> None:
    ...
    def test_mps_not_checked_when_cuda_available(
        self,
    ) -> None:
...
class TestCPUFallback:
    ...
    def test_cpu_fallback_when_no_cuda_or_mps(
        self,
    ) -> None:
    ...
    def test_cpu_fallback_returns_cpu_device_string(
        self,
    ) -> None:
    ...
    def test_cpu_fallback_when_cuda_and_mps_both_unavailable(
        self,
    ) -> None:
        """Test CPU fallback when both GPU options are unavailable."""
        with patch("but_with_subs.device.torch.cuda") as mock_cuda, patch(
            "but_with_subs.device.torch.backends.mps"
        ) as mock_mps:
            mock_cuda.is_available.return_value = False
...
class TestDeviceDetectionErrorHandling:
    ...
    def test_cuda_is_available_raises_exception(
        self,
    ) -> None:
    ...
    def test_mps_is_available_raises_exception_when_cuda_unavailable(
        self,
    ) -> None:
    ...
    def test_torch_device_raises_exception(
        self,
    ) -> None:
        """Test handling when torch.device() raises an exception."""
        with patch("but_with_subs.device.torch.cuda") as mock_cuda, patch(
            "but_with_subs.device.torch.device"
        ) as mock_device:
            mock_cuda.is_available.return_value = True
...
class TestDeviceSelectionPriority:
    ...
    def test_cuda_has_highest_priority(
        self,
    ) -> None:
    ...
    def test_cpu_has_lowest_priority(
        self,
    ) -> None:
...
class TestDeviceLogging:
    ...
    def test_device_is_logged(
        ...
        with patch("but_with_subs.device.torch.cuda") as mock_cuda:
            ...
            assert any(
                "Using device" in record.message for record in caplog.records
            )
