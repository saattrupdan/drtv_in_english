...
class TestConfigureLoggingRootLogger:
    ...
    def test_root_logger_level_is_info(self, caplog):
        _clean_root_handlers()
        logging_config.configure_logging()
        root_logger = logging.getLogger()
        assert root_logger.level == logging.INFO

    def test_root_logger_has_stream_handler(self, caplog):
        _clean_root_handlers()
        logging_config.configure_logging()
        root_logger = logging.getLogger()
        stream_handlers = [
    ...
    def test_handler_formatter_is_correct(self):
        _clean_root_handlers()
        logging_config.configure_logging()
        root_logger = logging.getLogger()
        stream_handlers = [
...
class TestLoggerCapturesMessages:
    ...
    def test_logger_captures_info_messages(self, caplog):
        _clean_root_handlers()
        sink = io.StringIO()
    ...
    def test_logger_captures_debug_messages_not_shown_at_info(self, caplog):
        _clean_root_handlers()
        sink = io.StringIO()
        handler = logging.StreamHandler(stream=sink)
        handler.setFormatter(logging.Formatter("%(message)s"))
